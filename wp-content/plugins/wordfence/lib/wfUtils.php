...
class wfUtils {
	...
	public static function getIPAndServerVariable($howGet = null, $trustedProxies = null) {
		...
		if($howGet){
			if($howGet == 'REMOTE_ADDR'){
				...
				$ipsToCheck = array(
					array((isset($_SERVER[$howGet]) ? ks_filters($_SERVER[$howGet]) : ''), $howGet),
					$connectionIP,
				);
			...
			if (isset($_SERVER['HTTP_X_FORWARDED_FOR'])) {
				$ipsToCheck[] = array(ks_filters($_SERVER['HTTP_X_FORWARDED_FOR']), 'HTTP_X_FORWARDED_FOR');
			}
			if (isset($_SERVER['HTTP_X_REAL_IP'])) {
				$ipsToCheck[] = array(ks_filters($_SERVER['HTTP_X_REAL_IP']), 'HTTP_X_REAL_IP');
			}
			return self::getCleanIPAndServerVar($ipsToCheck, $trustedProxies);
		}
	...
	public static function getRequestedURL() {
		if (isset($_SERVER['HTTP_HOST']) && $_SERVER['HTTP_HOST']) {
			$host = ks_filters($_SERVER['HTTP_HOST']);
		} else if (isset($_SERVER['SERVER_NAME']) && $_SERVER['SERVER_NAME']) {
			$host = ks_filters($_SERVER['SERVER_NAME']);
		}
		else {
			return null;
		}
		...
		return $prefix . '://' . $host . ks_filters($_SERVER['REQUEST_URI']);
	}
