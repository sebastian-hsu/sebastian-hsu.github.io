...
function ppb_text_image_func($atts, $content) {
	...
	if(!empty($display_gradient))
	{
		$return_html.= 'bg_gradient ';
	}
...
function ppb_header_image_func($atts, $content) {
	...
	if(!empty($width))
	{
		$return_html.= '<div style="width:'.esc_attr(rawurldecode($width)).';';
	...
	if(!empty($all_photo_arr) && is_array($all_photo_arr))
	{
		...
		foreach($all_photo_arr as $photo_id)
		{
		 	...
		 	if(isset($image_url[0]) && !empty($image_url[0]))
	    	{
				...
				{
		    		$return_html.= '<a class="fancy-gallery" data-rel="photography_gallery_'.esc_attr($gallery).'" '.photography_get_lightbox_caption_attr($photo_id).' href="'.esc_url($image_url[0]).'">';
	    		}
		...
		foreach($all_photo_arr as $photo_id)
		{
		 	...
		 	if(isset($image_url[0]) && !empty($image_url[0]))
	    	{
	    		$return_html.= '<a class="fancy-gallery" data-rel="photography_gallery_'.esc_attr($gallery).'" '.photography_get_lightbox_caption_attr($photo_id).' href="'.esc_url($image_url[0]).'">';
			    	...
			    	$return_html.= '<div class="gallery_image_wrapper">
				    	<img src="'.esc_url($small_image_url[0]).'" alt="'.esc_attr($image_alt).'" class="horizontal_gallery_img"/>';
			    	$return_html.= '</div>
	    	</a>';
...
function ppb_gallery_mixed_grid_func($atts, $content) {
	...
	if(!empty($all_photo_arr) && is_array($all_photo_arr))
	{
		...
		wp_enqueue_script("photography-script-gallery-metro-".$custom_id, admin_url('admin-ajax.php')."?action=photography_script_gallery_metro&id=".$custom_id."&gutter=".$gutter, false, THEMEVERSION, true);
		...
		foreach($all_photo_arr as $key => $photo_id)
		{
			...
			$return_html.= '<div class="'.esc_attr($column_class).' gallery_type filterable static'.($key+1).'">';
			
			$return_html.= '<a data-rel="photography_gallery_'.esc_attr($gallery_id).'" '.photography_get_lightbox_caption_attr($photo_id, true, $poster_image_url[0]).' href="'.esc_url($image_url[0]).'" '.photography_get_progressive_attr($photo_id, $small_image_url[0], $mobile_image_url[0], true, 'fancy-gallery').'><img src="'.esc_url($small_image_url[0]).'" alt="'.esc_attr($image_alt).'"/></a>';
...
function ppb_gallery_mixed_grid2_func($atts, $content) {
	...
	$return_html = '<div '.$sec_id.' class="ppb_gallery_mixed_grid2 '.$size.' nopadding ';
	...
	if(!empty($all_photo_arr) && is_array($all_photo_arr))
	{
		...
		//Register javascript
		wp_enqueue_script("photography-script-gallery-grid-".$custom_id, admin_url('admin-ajax.php')."?action=photography_script_gallery_grid&id=".$custom_id, false, THEMEVERSION, true);
		...
		$return_html.= '<div id="'.$custom_id.'" class="portfolio_mixed_filter_wrapper gallery portfolio-content section content clearfix ' .esc_attr($layout).'">';
...
add_shortcode('ppb_gallery_mixed_grid2', 'ppb_gallery_mixed_grid2_func');
...
function ppb_gallery_archive_func($atts, $content) {
	...
	if(!empty($galleris_arr) && is_array($galleris_arr))
	{
		...
		$return_html.= '<div id="'.$custom_id.'" class="portfolio_filter_wrapper portfolio-content content gallery '.esc_attr($wrapper_class).' ' .esc_attr($layout).'" data-columns="'.esc_attr($columns).'">';
		...
		foreach($galleris_arr as $key => $gallery)
		{
			...
			$return_html.= '<div class="element grid  ' .esc_attr($grid_wrapper_class).' '.esc_attr($hover_effect_class).'">';
			$return_html.= '<div class="'.esc_attr($column_class).' static filterable gallery_type animated'.esc_attr($key+1).' '.esc_attr($tg_gallery_archive_info_style).' archive">';
			...
			if(!empty($small_image_url[0]))
			{
			    ...
			    $return_html.= '<a href="'.esc_url($permalink_url).'" '.photography_get_lightbox_caption_attr($image_id, true, $poster_image_url[0]).' '.photography_get_progressive_attr($image_id, $small_image_url[0], $mobile_image_url[0], true, '').'><div class="gallery_archive_desc"><div class="gallery_archive_desc_content"><div class="gallery_archive_desc_inner">
							    <h4>'.$gallery->post_title.'</h4>
								<div class="post_detail">'.strip_tags($gallery->post_excerpt).'</div></div></div></div>';
				...
				$return_html.= '<img src="'.esc_url(photography_get_progressive_preview_image($image_id, 'photography-gallery-grid')).'" alt="'.esc_attr($gallery->post_title).'" class="preview"/></a>';
			}
...
function ppb_image_fixed_width_func($atts, $content) {
	...
	if(!empty($image))
	{
		...
		if(!empty($lightbox))
		{
			$return_html.= '<a href="'.esc_url($image).'" '.photography_get_lightbox_caption_attr($image_id, true, $poster_image_url[0]).' '.photography_get_progressive_attr($image_id, $image, $mobile_image_url[0], true, 'img_frame').'>';
		}
		
		$return_html.= '<img src="'.esc_url(photography_get_progressive_preview_image($image_id, 'original')).'" alt="'.esc_attr($image_alt).'" class="preview"/>';
...
function ppb_content_bg_func($atts, $content) {
	...
	$return_html = '<div '.$sec_id.' class="ppb_content_bg '.esc_attr($size.'_bg').' withsmallpadding ';
	...
	if(!empty($height))
	{
		$custom_css.= 'height: '.esc_attr($height).'px;';
	}
	...
	if(!empty($bgcolor))
	{
		...
		$return_html.= '<div class="ppb_background_overlay" style="'.esc_attr($custom_bgcolor_css.$custom_css_fontcolor).'"></div>';
	}
	
	$return_html.= '<div class="ppb_content_bg_content" style="padding:'.esc_attr($padding).'px;text-align:'.esc_attr($text_align).';">';
	
	//Add title and content
	if(!empty($title))
	{
		$return_html.= '<h2 class="ppb_content_bg_title" style="'.esc_attr($custom_css_fontcolor).'">'.rawurldecode(photography_get_first_title_word($title)).'</h2>';
	}
	
	if(!empty($subtitle))
	{
		$return_html.= '<div class="ppb_content_bg_subtitle" style="'.esc_attr($custom_css_fontcolor).'">'.rawurldecode($subtitle).'</div>';
	}
...
add_shortcode('ppb_content_bg', 'ppb_content_bg_func');
...
function ppb_image_half_fixed_width_func($atts, $content) {
	...
	if($align=='left')
	{
		...
		$return_html.= '<div class="one_half last content_middle">';
		if(!empty($content))
		{
			$return_html.= $content;
		}
	...
	{	
		$return_html.= '<div class="one_half content_middle textright">';
		if(!empty($content))
		{
			$return_html.= $content;
		}
...
function ppb_two_cols_images_func($atts, $content) {
	...
	if(!empty($image1))
	{
	    ...
	    if(!empty($lightbox))
	    {
	    	$return_html.= '<a href="'.esc_url($image1).'" '.photography_get_lightbox_caption_attr($image_id, true, $poster_image_url[0]).' '.photography_get_progressive_attr($image_id, $image1, $mobile_image_url[0], true, 'img_frame').'>';
	    }
	...
	if(!empty($image2))
	{
	    ...
	    if(!empty($lightbox))
	    {
	   	 $return_html.= '<a href="'.esc_url($image2).'" '.photography_get_lightbox_caption_attr($image_id, true, $poster_image_url[0]).' '.photography_get_progressive_attr($image_id, $image2, $mobile_image_url[0], true, 'img_frame').'>';
	   	}
...
function ppb_three_cols_images_func($atts, $content) {
	...
	if(!empty($image3))
	{
	    ...
	    if(!empty($lightbox))
	    {
	    	$return_html.= '<a href="'.esc_url($image3).'" '.photography_get_lightbox_caption_attr($image_id, true, $poster_image_url[0]).' '.photography_get_progressive_attr($image_id, $image3, $mobile_image_url[0], true, 'img_frame').'>';
	    }
...
function ppb_three_images_block_func($atts, $content) {
	...
	if($image_portrait_align=='left')
	{
		...
		if(!empty($image_portrait))
		{
		    ...
		    if(!empty($lightbox))
		    {
		    	$return_html.= '<a href="'.esc_url($image_portrait).'" '.photography_get_lightbox_caption_attr($image_id, true, $poster_image_url[0]).' '.photography_get_progressive_attr($image_id, $image_portrait, $mobile_image_url[0], true, 'img_frame').'>';
		    }
...
function ppb_four_images_block_func($atts, $content) {
	...
	if(!empty($image4))
	{
	    ...
	    if(!empty($lightbox))
		{
		 $return_html.= '<a href="'.esc_url($image4).'" '.photography_get_lightbox_caption_attr($image_id, true, $poster_image_url[0]).' '.photography_get_progressive_attr($image_id, $image4, $mobile_image_url[0], true, 'img_frame').'>';
		}
...
function ppb_portfolio_slider_func($atts, $content) {
	...
	if(!empty($portfolios_arr) && is_array($portfolios_arr))
	{
		...
		wp_enqueue_script("script-gallery-flexslider", admin_url('admin-ajax.php')."?action=photography_script_gallery_flexslider&autoplay=".$autoplay.'&amp;timer='.$timer, false, THEMEVERSION, true);
			...
			if(!empty($image_url[0]))
			{
			    ...
			    switch($portfolio_type)
			    {
	        	    ...
	        	    case 'Image':
						...
						$return_html.= '<a data-rel="photography_portfolio_'.esc_attr($custom_id).'" '.photography_get_portfolio_lightbox_caption_attr($portfolio->post_title).' href="'.esc_url($image_url[0]).'" class="fancy-gallery">';
				    ...
				    case 'Youtube Video':
						...
						$return_html.= '<a data-rel="photography_portfolio_'.esc_attr($custom_id).'" '.photography_get_portfolio_lightbox_caption_attr($portfolio->post_title, $image_url[0]).' href="https://www.youtube.com/embed/'.esc_attr($portfolio_video_id).'" class="lightbox_youtube" data-options="width:900, height:488">';
				...
				case 'Vimeo Video':
						...
						$return_html.= '<a data-rel="photography_portfolio_'.esc_attr($custom_id).'" '.photography_get_portfolio_lightbox_caption_attr($portfolio->post_title, $image_url[0]).' href="https://player.vimeo.com/video/'.esc_attr($portfolio_video_id).'?badge=0" class="lightbox_vimeo" data-options="width:900, height:506">';
				    ...
				    $preview_image = wp_get_attachment_image_src($image_id, 'large', true);
						...
						$return_html.= '<a data-rel="photography_portfolio_'.esc_attr($custom_id).'" '.photography_get_portfolio_lightbox_caption_attr($portfolio->post_title, $image_url[0]).' href="'.esc_url($portfolio_mp4_url).'" class="lightbox_vimeo">';
...
function ppb_portfolio_classic_func($atts, $content) {
	...
	wp_enqueue_script('masonry');
	wp_register_script("photography-script-gallery-masonry-".$custom_id, admin_url('admin-ajax.php')."?action=photography_script_gallery_masonry&id=".$custom_id."&gutter=".$gutter."&filter=".$filter."&next_action=photography_portfolio_classic&columns=3&type=grid&layout=".$layout."&page_id=".$current_page_id, false, THEMEVERSION, true);
	...
	wp_localize_script("photography-script-gallery-masonry-".$custom_id, 'tgAjax', $params );
	wp_enqueue_script("photography-script-gallery-masonry-".$custom_id, admin_url('admin-ajax.php')."?action=photography_script_gallery_masonry&id=".$custom_id."&gutter=".$gutter."&filter=".$filter."&next_action=photography_portfolio_classic&columns=3&type=grid&layout=".$layout."&page_id=".$current_page_id, false, THEMEVERSION, true);

	if($items_ini > 0)
	{
		wp_register_script("photography-script-portfolio-infinite-scroll-".$custom_id, admin_url('admin-ajax.php')."?action=photography_script_portfolio_infinite_scroll&id=".$custom_id.'&cat='.$cat.'&items='.$all_items.'&items_ini='.$items_ini.'&columns='.$columns.'&order='.$portfolio_order.'&order_by='.$portfolio_order_by.'&type=grid&gutter='.$gutter.'&layout='.$layout.'&filter='.$filter.'&next_action=photography_portfolio_classic&current_page_id='.$post->ID, false, THEMEVERSION, true);
		$params = array(
		  'ajaxurl' => admin_url('admin-ajax.php'),
		  'ajax_nonce' => wp_create_nonce('tgajax-post-contact-nonce'),
		);
	...
	if(!empty($portfolios_arr) && is_array($portfolios_arr))
	{
		...
		if($filterable == 'display')
		{
			...
			if($layout == 'wide' && ($tg_page_title_font_alignment == 'left' OR $tg_page_title_font_alignment == 'right'))
			{
				$return_html.= '<div class="standard_wrapper">';
			}
			...
			if(!empty($sets_arr))
			{
				$return_html.= '<ul id="portfolio_wall_filters_'.$custom_id.'" class="portfolio_wall_filters portfolio-main filter full">';
				$return_html.= '<li class="all-projects active">
		<a class="active" href="javascript:;" data-filter="">'.esc_html__('All', 'photography-custom-post' ).'</a>
		<span class="separator">/</span>
				...
				foreach($sets_arr as $key => $set_item)
				{
					...
					$return_html.= '<li class="cat-item '.esc_attr($set_item->slug).'" data-type="'.esc_attr($set_item->slug).'" style="clear:none">
						<a data-filter="'.esc_attr($set_item->slug).'" href="'.esc_attr($filter_link_url).'" title="'.esc_attr($set_item->name).'">'.$set_item->name.'</a>
						<span class="separator">/</span>
					</li>';
				}
		...
		foreach($portfolios_arr as $key => $portfolio)
		{
			...
			//Begin display HTML
			$return_html.= '<div class="element grid masonry '.esc_attr($grid_wrapper_class).' '.esc_attr($hover_effect_class).'">';
			...
			if(!empty($image_url[0]))
			{
			    ...
			    switch($portfolio_type)
			    {
				    case 'External Link':
						...
						$return_html.= '<a target="_blank" href="'.esc_url($portfolio_link_url).'" '.photography_get_progressive_attr($image_id, $small_image_url[0], $mobile_image_url[0], true).'><img src="'.esc_url(photography_get_progressive_preview_image($image_id, 'photography-gallery-grid')).'" alt="'.esc_attr($portfolio->post_title).'" class="preview"/><div class="portfolio_classic_icon_wrapper">
	        	    ...
	        	    default:
			        	...
			        	$return_html.= '<a href="'.get_permalink($portfolio_ID).'" '.photography_get_progressive_attr($image_id, $small_image_url[0], $mobile_image_url[0], true).'><img src="'.esc_url(photography_get_progressive_preview_image($image_id, 'photography-gallery-grid')).'" alt="'.esc_attr($portfolio->post_title).'" class="preview"/><div class="portfolio_classic_icon_wrapper">
								<div class="portfolio_classic_icon_content">
	        	    ...
	        	    case 'Image':
						...
						$return_html.= '<a data-rel="photography_portfolio_'.esc_attr($custom_id).'" '.photography_get_portfolio_lightbox_caption_attr($portfolio->post_title).' href="'.esc_url($image_url[0]).'" '.photography_get_progressive_attr($image_id, $small_image_url[0], $mobile_image_url[0], true, 'fancy-gallery').'><img src="'.esc_url(photography_get_progressive_preview_image($image_id, 'photography-gallery-grid')).'" alt="'.esc_attr($portfolio->post_title).'" class="preview"/><div class="portfolio_classic_icon_wrapper">
								<div class="portfolio_classic_icon_content">
				    ...
				    case 'Youtube Video':
						...
						$return_html.= '<a data-rel="photography_portfolio_'.esc_attr($custom_id).'" '.photography_get_portfolio_lightbox_caption_attr($portfolio->post_title, $image_url[0]).' href="https://www.youtube.com/embed/'.esc_attr($portfolio_video_id).'" '.photography_get_progressive_attr($image_id, $small_image_url[0], $mobile_image_url[0], true, 'lightbox_youtube').' data-options="width:900, height:488"><img src="'.esc_url(photography_get_progressive_preview_image($image_id, 'photography-gallery-grid')).'" alt="'.esc_attr($portfolio->post_title).'" class="preview"/><div class="portfolio_classic_icon_wrapper">
								<div class="portfolio_classic_icon_content">
				...
				case 'Vimeo Video':
						...
						$return_html.= '<a data-rel="photography_portfolio_'.esc_attr($custom_id).'" '.photography_get_portfolio_lightbox_caption_attr($portfolio->post_title, $image_url[0]).' href="https://player.vimeo.com/video/'.esc_attr($portfolio_video_id).'?badge=0" '.photography_get_progressive_attr($image_id, $small_image_url[0], $mobile_image_url[0], true, 'lightbox_vimeo').' data-options="width:900, height:506"><img src="'.esc_url(photography_get_progressive_preview_image($image_id, 'photography-gallery-grid')).'" alt="'.esc_attr($portfolio->post_title).'" class="preview"/><div class="portfolio_classic_icon_wrapper">
								<div class="portfolio_classic_icon_content">
									<i class="fa fa-play"></i>
								</div>
							</div></a>';
				...
				case 'Self-Hosted Video':
				    ...
				    $preview_image = wp_get_attachment_image_src($image_id, 'large', true);
						...
						$return_html.= '<a data-rel="photography_portfolio_'.esc_attr($custom_id).'" '.photography_get_portfolio_lightbox_caption_attr($portfolio->post_title, $image_url[0]).' title="'.esc_attr($portfolio->post_title).'" href="'.esc_url($portfolio_mp4_url).'" '.photography_get_progressive_attr($image_id, $small_image_url[0], $mobile_image_url[0], true, 'lightbox_vimeo').'><img src="'.esc_url(photography_get_progressive_preview_image($image_id, 'photography-gallery-grid')).'" alt="'.esc_attr($portfolio->post_title).'" class="preview"/><div class="portfolio_classic_icon_wrapper">
								<div class="portfolio_classic_icon_content">
									<i class="fa fa-play"></i>
								</div>
							</div></a>';
			...
			//Display portfolio detail
			$return_html.= '<div id="portfolio_desc_'.esc_attr($portfolio_ID).'" class="portfolio_desc portfolio'.esc_attr($columns).' ' .esc_attr($layout).' filterable">';
...
function ppb_portfolio_classic_masonry_func($atts, $content) {
	...
	wp_enqueue_script('masonry');
	wp_register_script("photography-script-gallery-masonry-".$custom_id, admin_url('admin-ajax.php')."?action=photography_script_gallery_masonry&id=".$custom_id."&gutter=".$gutter."&filter=".$filter."&next_action=photography_portfolio_classic&columns=3&type=masonry&layout=".$layout."&page_id=".$current_page_id, false, THEMEVERSION, true);
	...
	wp_localize_script("photography-script-gallery-masonry-".$custom_id, 'tgAjax', $params );
	wp_enqueue_script("photography-script-gallery-masonry-".$custom_id, admin_url('admin-ajax.php')."?action=photography_script_gallery_masonry&id=".$custom_id."&gutter=".$gutter."&filter=".$filter."&next_action=photography_portfolio_classic&columns=3&type=masonry&layout=".$layout."&page_id=".$current_page_id, false, THEMEVERSION, true);
	
	if($items_ini > 0)
	{
		wp_register_script("photography-script-portfolio-infinite-scroll-".$custom_id, admin_url('admin-ajax.php')."?action=photography_script_portfolio_infinite_scroll&id=".$custom_id.'&cat='.$cat.'&items='.$all_items.'&items_ini='.$items_ini.'&columns='.$columns.'&order='.$portfolio_order.'&order_by='.$portfolio_order_by.'&type=masonry&gutter='.$gutter.'&layout='.$layout.'&filter='.$filter.'&next_action=photography_portfolio_classic&current_page_id='.$current_page_id, false, THEMEVERSION, true);
		$params = array(
		  'ajaxurl' => admin_url('admin-ajax.php'),
		  'ajax_nonce' => wp_create_nonce('tgajax-post-contact-nonce'),
		);
	...
	if(!empty($portfolios_arr) && is_array($portfolios_arr))
	{
		...
		foreach($portfolios_arr as $key => $portfolio)
		{
			...
			if(!empty($image_url[0]))
			{
			    ...
			    switch($portfolio_type)
			    {
			    	case 'External Link':
						...
						$return_html.= '<a target="_blank" href="'.esc_url($portfolio_link_url).'" '.photography_get_progressive_attr($image_id, $small_image_url[0], $mobile_image_url[0], true).'><img src="'.esc_url(photography_get_progressive_preview_image($image_id, 'photography-gallery-masonry')).'" alt="'.esc_attr($portfolio->post_title).'" class="preview"/><div class="portfolio_classic_icon_wrapper">
								<div class="portfolio_classic_icon_content">
									<i class="fa fa-chain"></i>
								</div>
							</div></a>';
			...
			//Display portfolio detail
			$return_html.= '<div id="portfolio_desc_'.$portfolio_ID.'" class="portfolio_desc portfolio'.$columns.' ' .esc_attr($layout).' filterable">';
            $return_html.= '<h5>'.$portfolio->post_title.'</h5>';
            $return_html.= '<div class="post_detail">'.$portfolio->post_excerpt.'</div>';
			$return_html.= '</div>';
			$return_html.= '</div>';
...
function ppb_portfolio_grid_func($atts, $content) {
	...
	wp_enqueue_script('masonry');
	wp_register_script("photography-script-gallery-masonry-".$custom_id, admin_url('admin-ajax.php')."?action=photography_script_gallery_masonry&id=".$custom_id."&gutter=".$gutter."&filter=".$filter."&next_action=photography_portfolio_grid&columns=3&type=grid&layout=".$layout."&page_id=".$current_page_id, false, THEMEVERSION, true);
...
wp_localize_script("photography-script-gallery-masonry-".$custom_id, 'tgAjax', $params );
wp_enqueue_script("photography-script-gallery-masonry-".$custom_id, admin_url('admin-ajax.php')."?action=photography_script_gallery_masonry&id=".$custom_id."&gutter=".$gutter."&filter=".$filter."&next_action=photography_portfolio_grid&columns=3&type=grid&layout=".$layout."&page_id=".$current_page_id, false, THEMEVERSION, true);

	if($items_ini > 0)
	{
		wp_register_script("photography-script-portfolio-infinite-scroll-".$custom_id, admin_url('admin-ajax.php')."?action=photography_script_portfolio_infinite_scroll&id=".$custom_id.'&cat='.$cat.'&items='.$all_items.'&items_ini='.$items_ini.'&columns='.$columns.'&order='.$portfolio_order.'&order_by='.$portfolio_order_by.'&type=grid&gutter='.$gutter.'&layout='.$layout.'&filter='.$filter.'&next_action=photography_portfolio_grid&current_page_id='.$current_page_id, false, THEMEVERSION, true);
		$params = array(
		  'ajaxurl' => admin_url('admin-ajax.php'),
		  'ajax_nonce' => wp_create_nonce('tgajax-post-contact-nonce'),
		);
			...
			if(!empty($image_url[0]))
			{
			    ...
			    switch($portfolio_type)
			    {
				    case 'External Link':
						...
						$return_html.= '<a target="_blank" href="'.esc_url($portfolio_link_url).'" '.photography_get_progressive_attr($image_id, $small_image_url[0], $mobile_image_url[0], true).'><img src="'.esc_url(photography_get_progressive_preview_image($image_id, 'photography-gallery-grid')).'" alt="'.esc_attr($portfolio->post_title).'" class="preview"/><div id="portfolio_desc_'.esc_attr($portfolio_ID).'" class="portfolio_title">
	        					<div class="table">
	        						<div class="cell">
...
function ppb_portfolio_mixed_grid_func($atts, $content) {
	...
	//Register javascript
	wp_enqueue_script('masonry');
	wp_register_script("photography-script-gallery-metro-".$custom_id, admin_url('admin-ajax.php')."?action=photography_script_gallery_metro&id=".$custom_id."&gutter=".$gutter."&filter=".$filter."&next_action=photography_portfolio_metro&layout=".$layout."&page_id=".$custom_id, false, THEMEVERSION, true);
	...
	wp_localize_script("photography-script-gallery-metro-".$custom_id, 'tgAjax', $params );
	wp_enqueue_script("photography-script-gallery-metro-".$custom_id, admin_url('admin-ajax.php')."?action=photography_script_gallery_metro&id=".$custom_id."&gutter=".$gutter."&filter=".$filter."&next_action=photography_portfolio_metro&layout=".$layout."&page_id=".$custom_id, false, THEMEVERSION, true);
			...
			if(!empty($image_url[0]))
			{
			    ...
			    switch($portfolio_type)
			    {
	        	    ...
	        	    case 'Image':
						...
						$return_html.= '<a data-rel="photography_portfolio_'.esc_attr($custom_id).'" '.photography_get_portfolio_lightbox_caption_attr($portfolio->post_title).' href="'.esc_url($image_url[0]).'" class="fancy-gallery"><img src="'.esc_url($small_image_url[0]).'" alt="'.esc_attr($portfolio->post_title).'"/><div id="portfolio_desc_'.esc_attr($portfolio_ID).'" class="portfolio_title">
	        					<div class="table">
	        						<div class="cell">
							            <h5>'.$portfolio->post_title.'</h5>
							            <div class="post_detail">'.$portfolio->post_excerpt.'</div>
				    ...
				    case 'Youtube Video':
						...
						$return_html.= '<a data-rel="photography_portfolio_'.esc_attr($custom_id).'" '.photography_get_portfolio_lightbox_caption_attr($portfolio->post_title, $image_url[0]).' href="https://www.youtube.com/embed/'.esc_attr($portfolio_video_id).'" class="lightbox_youtube"><img src="'.esc_url($small_image_url[0]).'" alt="'.esc_attr($portfolio->post_title).'"/><div id="portfolio_desc_'.esc_attr($portfolio_ID).'" class="portfolio_title">
	        					<div class="table">
	        						<div class="cell">
							            <h5>'.$portfolio->post_title.'</h5>
							            <div class="post_detail">'.$portfolio->post_excerpt.'</div>
				...
				case 'Vimeo Video':
						...
						$return_html.= '<a data-rel="photography_portfolio_'.esc_attr($custom_id).'" '.photography_get_portfolio_lightbox_caption_attr($portfolio->post_title, $image_url[0]).' href="https://player.vimeo.com/video/'.esc_attr($portfolio_video_id).'?badge=0" class="lightbox_vimeo"><img src="'.esc_url($small_image_url[0]).'" alt="'.esc_attr($portfolio->post_title).'"/><div id="portfolio_desc_'.esc_attr($portfolio_ID).'" class="portfolio_title">
	        					<div class="table">
	        						<div class="cell">
							            <h5>'.$portfolio->post_title.'</h5>
							            <div class="post_detail">'.$portfolio->post_excerpt.'</div>
				...
				case 'Self-Hosted Video':
				    ...
				    $preview_image = wp_get_attachment_image_src($image_id, 'large', true);
						...
						$return_html.= '<a data-rel="photography_portfolio_'.esc_attr($custom_id).'" '.photography_get_portfolio_lightbox_caption_attr($portfolio->post_title, $image_url[0]).' href="'.esc_url($portfolio_mp4_url).'" class="lightbox_vimeo"><img src="'.esc_url($small_image_url[0]).'" alt="'.esc_attr($portfolio->post_title).'"/><div id="portfolio_desc_'.esc_attr($portfolio_ID).'" class="portfolio_title">
	        					<div class="table">
	        						<div class="cell">
							            <h5>'.$portfolio->post_title.'</h5>
							            <div class="post_detail">'.$portfolio->post_excerpt.'</div>
...
function ppb_portfolio_mixed_grid2_func($atts, $content) {
	...
	$return_html = '<div '.$sec_id.' class="ppb_mixed_portfolio2 '.$size.' nopadding ';
	...
	//Register javascript
	wp_enqueue_script('masonry');
	wp_register_script("photography-script-gallery-mixed-grid-".$custom_id, admin_url('admin-ajax.php')."?action=photography_script_gallery_mixed_grid&id=".$custom_id."&gutter=".$gutter."&filter=".$filter."&next_action=photography_portfolio_mixed_grid&layout=".$layout."&page_id=".$custom_id, false, THEMEVERSION, true);
	...
	wp_localize_script("photography-script-gallery-mixed-grid-".$custom_id, 'tgAjax', $params );
	wp_enqueue_script("photography-script-gallery-mixed-grid-".$custom_id, admin_url('admin-ajax.php')."?action=photography_script_gallery_mixed_grid&id=".$custom_id."&gutter=".$gutter."&filter=".$filter."&next_action=photography_portfolio_mixed_grid&layout=".$layout."&page_id=".$custom_id, false, THEMEVERSION, true);
	...
	if(!empty($portfolios_arr) && is_array($portfolios_arr))
	{
		...
		$return_html.= '<div id="'.$custom_id.'" class="portfolio_mixed_filter_wrapper gallery portfolio-content section content clearfix '.esc_attr($wrapper_class).' ' .esc_attr($layout).' mixed_grid" data-items="'.esc_attr($all_items).'" data-cat="'.esc_attr($cat).'">';
...
add_shortcode('ppb_portfolio_mixed_grid2', 'ppb_portfolio_mixed_grid2_func');
...
function ppb_portfolio_grid_masonry_func($atts, $content) {
	...
	wp_enqueue_script('masonry');
	wp_register_script("photography-script-gallery-masonry-".$custom_id, admin_url('admin-ajax.php')."?action=photography_script_gallery_masonry&id=".$custom_id."&gutter=".$gutter."&filter=".$filter."&next_action=photography_portfolio_grid&columns=3&type=masonry&layout=".$layout."&page_id=".$current_page_id, false, THEMEVERSION, true);
	...
	wp_localize_script("photography-script-gallery-masonry-".$custom_id, 'tgAjax', $params );
	wp_enqueue_script("photography-script-gallery-masonry-".$custom_id, admin_url('admin-ajax.php')."?action=photography_script_gallery_masonry&id=".$custom_id."&gutter=".$gutter."&filter=".$filter."&next_action=photography_portfolio_grid&columns=3&type=masonry&layout=".$layout."&page_id=".$current_page_id, false, THEMEVERSION, true);

	if($items_ini > 0)
	{
		wp_register_script("photography-script-portfolio-infinite-scroll-".$custom_id, admin_url('admin-ajax.php')."?action=photography_script_portfolio_infinite_scroll&id=".$custom_id.'&cat='.$cat.'&items='.$all_items.'&items_ini='.$items_ini.'&columns='.$columns.'&order='.$portfolio_order.'&order_by='.$portfolio_order_by.'&type=masonry&gutter='.$gutter.'&layout='.$layout.'&filter='.$filter.'&next_action=photography_portfolio_grid&current_page_id='.$current_page_id, false, THEMEVERSION, true);
		$params = array(
		  'ajaxurl' => admin_url('admin-ajax.php'),
		  'ajax_nonce' => wp_create_nonce('tgajax-post-contact-nonce'),
		);
		    ...
		    switch($post_ft_type)
		    {
		    	...
		    	default:
		    	    ...
		    	    $return_html.= '<div class="post_img small">
		    	    	<a href="'.esc_url(get_permalink($ppb_post->ID)).'" '.photography_get_progressive_attr($image_id, $small_image_url[0], '', false).'>
		    	    		<img src="'.esc_url(photography_get_progressive_preview_image($image_id, 'photography-blog')).'" alt="'.esc_attr($ppb_post->post_title).'" class="preview"/>
		                </a>
		    	    </div>';
		    		}
		    	break;
		...
		foreach($posts_arr as $key => $ppb_post)
		{
		    ...
		    switch($post_ft_type)
		    {
		    	...
		    	default:
		    	    ...
		    	    $return_html.= '<div class="post_img small">
		    	    	<a href="'.esc_url(get_permalink($ppb_post->ID)).'" '.photography_get_progressive_attr($image_id, $small_image_url[0], '', false).'>
		    	    		<img src="'.esc_url(photography_get_progressive_preview_image($image_id, 'photography-gallery-masonry')).'" alt="'.esc_attr($ppb_post->post_title).'" class="preview"/>
		                </a>
		    	    </div>';
		    		}
		    	break;
...
function ppb_team_column_func($atts, $content) {
	...
	if(!empty($team_arr) && is_array($team_arr))
	{
		...
		foreach($team_arr as $key => $member)
		{
			...
			$member_instagram = get_post_meta($member_ID, 'member_instagram', true);
            
            if(!empty($member_facebook) OR !empty($member_twitter) OR !empty($member_google) OR !empty($member_linkedin) OR !empty($member_instagram))
			{
			    ...
			    {
			        $return_html.= '<li><a title="'.esc_attr($member->post_title).' on Instagram" target="_blank" class="tooltip" href="'.$member_instagram.'" '.$social_font_color.'><i class="fa fa-instagram"></i></a></li>';
			    }
...
function ppb_testimonial_slider_func($atts, $content) {
	...
	wp_enqueue_script("flexslider-js", get_template_directory_uri()."/js/flexslider/jquery.flexslider-min.js", false, THEMEVERSION, true);
	wp_enqueue_script("sciprt-testimonials-flexslider", get_template_directory_uri()."/js/custom_testimonial_slider.js", false, THEMEVERSION, true);
...
function ppb_contact_map_func($atts, $content) {
    ...
    wp_register_script("script-contact-form", admin_url('admin-ajax.php')."?action=photography_script_contact_form&form=".$custom_id, false, THEMEVERSION, true);
	$params = array(
	  'ajaxurl' => admin_url('admin-ajax.php'),
	  'ajax_nonce' => wp_create_nonce('tgajax-post-contact-nonce'),
	);
	wp_localize_script( 'script-contact-form', 'tgAjax', $params );
	wp_enqueue_script("script-contact-form", admin_url('admin-ajax.php')."?action=photography_script_contact_form&form=".$custom_id, false, THEMEVERSION, true);
    ...
    if(is_array($pp_contact_form) && !empty($pp_contact_form))
    {
    	...
    	$return_html.= '<div id="captcha-wrap">
    		<div class="captcha-box">
    			<img src="'.admin_url('admin-ajax.php').'?action=photography_script_get_captcha" alt="" id="captcha" />
    		</div>
    		<div class="text-box">
	...
	wp_enqueue_script("script-contact-map".$custom_id, admin_url('admin-ajax.php')."?action=photography_script_map_shortcode&fullheight=true&data=".$ext_attr_serialize, false, THEMEVERSION, true);
	$return_html.= '</div>';
...
function ppb_map_func($atts) {
	...
	wp_enqueue_script("simplegmaps", get_template_directory_uri()."/js/jquery.simplegmaps.min.js", false, THEMEVERSION, true);
	wp_enqueue_script("script-contact-map".$custom_id, admin_url('admin-ajax.php')."?action=photography_script_map_shortcode&data=".$ext_attr_serialize, false, THEMEVERSION, true);
...
function ppb_animated_gallery_grid_func($atts, $content) {
	...
	if(!empty($images_arr))
	{
		...
		wp_enqueue_script("photography-script-gridrotator-".$custom_id, admin_url('admin-ajax.php')."?action=photography_script_gridrotator&grid=".$custom_id.'&rows='.$rows, false, THEMEVERSION, true);
...
function ppb_client_column_func($atts, $content) {
	...
	if(!empty($image5))
	{
		...
		$image_id = photography_get_image_id($image5);
		$image_alt = get_post_meta($image_id, '_wp_attachment_image_alt', true);
...
add_shortcode('ppb_client_column', 'ppb_client_column_func');
