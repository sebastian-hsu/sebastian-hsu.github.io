...
function tg_social_icons_func($atts, $content) {
	...
	{
		$return_html.='<li class="twitter"><a target="_blank" title="Twitter" href="//twitter.com/'.$pp_twitter_username.'"><i class="fa fa-twitter"></i></a></li>';
	}
	...
	{
		$return_html.='<li class="flickr"><a target="_blank" title="Flickr" href="//flickr.com/people/'.esc_attr($pp_flickr_username).'"><i class="fa fa-flickr"></i></a></li>';
	}
	...
	{
		$return_html.='<li class="vimeo"><a target="_blank" title="Vimeo" href="//vimeo.com/'.$pp_vimeo_username.'"><i class="fa fa-vimeo-square"></i></a></li>';
	}

	$pp_tumblr_username = get_option('pp_tumblr_username');
	if(!empty($pp_tumblr_username))
	{
		$return_html.='<li class="tumblr"><a target="_blank" title="Tumblr" href="//'.$pp_tumblr_username.'.tumblr.com"><i class="fa fa-tumblr"></i></a></li>';
	}
	...
	{
		$return_html.='<li class="dribbble"><a target="_blank" title="Dribbble" href="//dribbble.com/'.$pp_dribbble_username.'"><i class="fa fa-dribbble"></i></a></li>';
	}
	...
	{
		$return_html.='<li class="pinterest"><a target="_blank" title="Pinterest" href="//pinterest.com/'.$pp_pinterest_username.'"><i class="fa fa-pinterest"></i></a></li>';
	}
		        	
	$pp_instagram_username = get_option('pp_instagram_username');
	if(!empty($pp_instagram_username))
	{
		$return_html.='<li class="instagram"><a target="_blank" title="Instagram" href="//instagram.com/'.strtolower($pp_instagram_username).'"><i class="fa fa-instagram"></i></a></li>';
	}
	
	$pp_behance_username = get_option('pp_behance_username');
    if(!empty($pp_behance_username))
	{
		$return_html.='<li class="behance"><a target="_blank" title="Behance" href="//behance.net/'.$pp_behance_username.'"><i class="fa fa-behance-square"></i></a></li>';
	}
...
function tg_map_func($atts) {
	...
	wp_enqueue_script("photography-script-contact-map".$custom_id, admin_url('admin-ajax.php')."?action=photography_script_map_shortcode&amp;data=".$ext_attr_serialize, false, THEMEVERSION, true);
...
function tg_grid_gallery_func($atts, $content) {
	...
	//Register javascript
	wp_enqueue_script("script-gallery-grid-".$custom_id, admin_url('admin-ajax.php')."?action=photography_script_gallery_grid&id=".$custom_id, false, THEMEVERSION, true);
	
	//Check if enable infinite scroll
	if($items > 0)
	{
		wp_register_script("photography-script-gallery-infinite-scroll-".$custom_id, admin_url('admin-ajax.php')."?action=photography_script_gallery_infinite_scroll&id=".$custom_id.'&gallery_id='.$gallery_id.'&items='.$items.'&columns='.$columns, false, THEMEVERSION, true);
		$params = array(
		...
		wp_enqueue_script("photography-script-gallery-infinite-scroll-".$custom_id, admin_url('admin-ajax.php')."?action=photography_script_gallery_infinite_scroll&id=".$custom_id.'&gallery_id='.$gallery_id.'&items='.$items.'&columns='.$columns.'&type=grid', false, THEMEVERSION, true);
	}
	...
	if(!empty($images_arr))
	{
		...
		foreach($images_arr as $key => $image)
		{
			if ($items == 0 OR $i < $items) 
			{
				...
				$return_html.= '<img src="'.esc_url(photography_get_progressive_preview_image($image, 'photography-gallery-grid')).'" alt="'.esc_attr($image_alt).'" class="preview"/>';
...
function tg_masonry_gallery_func($atts, $content) {
	...
	wp_enqueue_script("photography-script-gallery-masonry-".$custom_id, admin_url('admin-ajax.php')."?action=photography_script_gallery_masonry&id=".$custom_id."&gutter=".$gutter, false, THEMEVERSION, true);
	
	//Check if enable infinite scroll
	if($items > 0)
	{
		wp_register_script("photography-script-gallery-infinite-scroll-".$custom_id, admin_url('admin-ajax.php')."?action=photography_script_gallery_infinite_scroll&id=".$custom_id.'&gallery_id='.$gallery_id.'&items='.$items.'&columns='.$columns.'&type=masonry', false, THEMEVERSION, true);
		$params = array(
		  'ajaxurl' => admin_url('admin-ajax.php'),
		  'ajax_nonce' => wp_create_nonce('tgajax-post-contact-nonce'),
		);
		...
		wp_localize_script("photography-script-gallery-infinite-scroll-".$custom_id, 'tgAjax', $params );
		wp_enqueue_script("photography-script-gallery-infinite-scroll-".$custom_id, admin_url('admin-ajax.php')."?action=photography_script_gallery_infinite_scroll&id=".$custom_id.'&gallery_id='.$gallery_id.'&items='.$items.'&columns='.$columns.'&type=masonry', false, THEMEVERSION, true);
	}
	...
	if(!empty($images_arr))
	{
		...
		foreach($images_arr as $key => $image)
		{
			if ($items == 0 OR $i < $items) 
			{
				...
				$return_html.= '<div class="element masonry  ' .esc_attr($grid_wrapper_class).' '.esc_attr($hover_effect_class).'">';
				$return_html.= '<div class="'.esc_attr($column_class).' static filterable gallery_type animated'.esc_attr($key+1).'">';
				$return_html.= '<a data-rel="photography_gallery_'.esc_attr($gallery_id).'" '.photography_get_lightbox_caption_attr($image, true, $poster_image_url[0]).' href="'.esc_url($image_url[0]).'" '.photography_get_progressive_attr($image, $small_image_url[0], $mobile_image_url[0], true, 'fancy-gallery').'>';
				$return_html.= '<img src="'.esc_url(photography_get_progressive_preview_image($image, 'photography-gallery-masonry')).'" alt="'.esc_attr($image_alt).'" class="preview"/>';
...
function tg_testimonial_slider_func($atts, $content) {
	...
	wp_enqueue_script("photography-sciprt-testimonials-flexslider", get_template_directory_uri()."/js/custom_testimonial_slider.js", false, THEMEVERSION, true);
...
function tg_lightbox_func($atts, $content) {
	...
	if($tg_lightbox_plugin == 'ilightbox')
	{
		if($type == 'youtube')
		{
			$href = 'http://www.youtube.com/embed/'.$youtube_id.'?autohide=1&border=0&egm=0&showinfo=0&showsearch=0';
			$class = 'lightbox_youtube';
		}
	
		if($type == 'vimeo')
		{
			$href = 'http://player.vimeo.com/video/'.$vimeo_id;
...
function tg_animate_counter_func($atts, $content) {
	...
	wp_enqueue_script("photography-script-animate-counter".$custom_id, admin_url('admin-ajax.php')."?action=photography_script_animate_counter&id=".$custom_id."&start=".$start."&end=".$end."&fontsize=".$fontsize, false, THEMEVERSION, true);
...
function tg_animate_circle_func($atts, $content) {
	...
	wp_enqueue_script("photography-script-animate-circle".$custom_id, admin_url('admin-ajax.php')."?action=photography_script_animate_circle&id=".$custom_id, false, THEMEVERSION, true);
...
function tg_gallery_slider_func($atts, $content) {
	...
	wp_enqueue_script("photography-flexslider-js", get_template_directory_uri()."/js/flexslider/jquery.flexslider-min.js", false, THEMEVERSION, true);
	wp_enqueue_script("photography-script-gallery-flexslider", admin_url('admin-ajax.php')."?action=photography_script_gallery_flexslider&autoplay=".$autoplay.'&amp;caption='.$caption.'&amp;timer='.$timer, false, THEMEVERSION, true);
