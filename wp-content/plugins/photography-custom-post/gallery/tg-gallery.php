...
class wpsimplegallery {
    ...
    public function ajax_get_thumbnail() {
        ...
        echo $this->admin_thumb(ks_filters($_POST['imageid']));
        die;
    }

    public function ajax_get_all_attachments() {
        $post_id = ks_filters($_POST['post_id']);
        $included = (isset(($_POST['included']))) ? ks_filters($_POST['included']) : array();
    ...
    public function save_post_meta($post_id) {
        ...
        if (!isset(($_POST['wpsimplegallery_gallery_nonce'])) || !wp_verify_nonce(ks_filters($_POST['wpsimplegallery_gallery_nonce']), basename(__FILE__)))
            return (isset($post_id)) ? $post_id : 0;

        $images = (isset(($_POST['wpsimplegallery_thumb']))) ? ks_filters($_POST['wpsimplegallery_thumb']) : array();
        $gallery = array();
        if (count($images) > 0) {
