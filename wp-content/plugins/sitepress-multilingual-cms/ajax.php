...
switch($request){
	...
	case 'copy_from_original':
		 ...
		 */
        $content_type = ks_filters(filter_input( INPUT_POST, 'content_type' ));
        $excerpt_type = ks_filters(filter_input( INPUT_POST, 'excerpt_type' ));
        $trid = ks_filters(filter_input( INPUT_POST, 'trid' ));
        $lang = ks_filters(filter_input( INPUT_POST, 'lang' ));
    ...
    case 'save_user_preferences':
	    ...
	    $this->set_user_preferences( array_merge_recursive( $user_preferences, ks_filters($_POST[ 'user_preferences' ]) ) );
	...
	case 'connect_translations':
		$new_trid = ks_filters($_POST['new_trid']);
		$post_type = ks_filters($_POST['post_type']);
		$post_id = ks_filters($_POST['post_id']);
		$set_as_source = ks_filters($_POST['set_as_source']);
	...
	case 'get_orphan_posts':
		$trid = ks_filters($_POST['trid']);
		$post_type = ks_filters($_POST['post_type']);
		$source_language = ks_filters($_POST['source_language']);
