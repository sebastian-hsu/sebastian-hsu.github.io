...
function wpml_get_authenticated_action() {
	...
	$action = ks_filters(filter_input( INPUT_POST, 'icl_ajx_action' ));
	$action = $action ? $action : ks_filters(filter_input( INPUT_POST, 'action' ));
	$nonce  = $action ? ks_filters(filter_input( INPUT_POST, '_icl_nonce' )) : null;
	if ( $nonce === null || $action === null ) {
		$action = ks_filters(filter_input( INPUT_GET, 'icl_ajx_action' ));
		$nonce  = $action ? ks_filters(filter_input( INPUT_GET, '_icl_nonce' )) : null;
	}
...
function wpml_is_action_authenticated( $action ) {
	$nonce = isset( $_POST['_icl_nonce'] ) ? ks_filters($_POST['_icl_nonce']) : '';
	if ( '' !== $nonce ) {
		$action = $action . '_nonce';
	} else {
		$nonce = isset( $_POST['nonce'] ) ? ks_filters($_POST['nonce']) : '';
	}
