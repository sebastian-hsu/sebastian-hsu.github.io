jQuery(document).ready(function($) {
	...
	var toShow = jQuery( '#wp-timeline .subsubsub li:first-child a.tab' ).attr( 'href' );
	jQuery( '#wp-timeline .subsubsub li:first-child a.tab' ).addClass( 'current' );
	// Remove the first occurance of # from the selected string (will be added manually below).
	toShow = toShow.replace( '#', '');

	jQuery( '#wp-timeline h2, #wp-timeline form > p:not(".submit"), #wp-timeline table' ).hide();
	jQuery( 'h2#' + toShow ).show().nextUntil( 'h2.section-heading', 'p, table, table p' ).show();
    // Create nav links for settings page
    jQuery( '#wp-timeline .subsubsub a.tab' ).click( function ( e ) {
        ...
        //if ( jQuery( this ).hasClass( 'all' ) ) {
            //jQuery( '#wp-timeline h2, #wp-timeline form p, #wp-timeline table.form-table, p.submit' ).show();
