EXC_MB.addCallbackForDeletedField( 'EXC_MB_wysiwyg', function( el ) {

	// Destroy WYSIWYG editors instances.
	el.find( '.exc_mb-wysiwyg textarea' ).each( function() {
...
EXC_MB.addCallbackForClonedField( 'EXC_MB_wysiwyg', function( newT ) {

	newT.find( '.exc_mb-wysiwyg' ).each( function (i) {
		...
		nameRegex = new RegExp( 'exc_mb-placeholder-name-' + fieldId, 'g' );
		idRegex   = new RegExp( 'exc_mb-placeholder-id-' + fieldId, 'g' );

		// Placeholder markup for the new wysiwyg is stored as a prop on var exc_mb_wysiwyg_editors
		// Copy, update ids & names & insert.
		el.html( exc_mb_wysiwyg_editors[fieldId].replace( nameRegex, name ).replace( idRegex, id ) );

		// If no settings for this field. Clone from placeholder.
		if ( typeof( tinyMCEPreInit.mceInit[ id ] ) === 'undefined' ) {
			var newSettings = jQuery.extend( {}, tinyMCEPreInit.mceInit[ 'exc_mb-placeholder-id-' + fieldId ] );
		...
		if ( typeof( tinyMCEPreInit.qtInit[ id ] ) === 'undefined' ) {
			var newQTS = jQuery.extend( {}, tinyMCEPreInit.qtInit[ 'exc_mb-placeholder-id-' + fieldId ] );
...
EXC_MB.addCallbackForSortStart( 'EXC_MB_wysiwyg', function( el ) {
...
EXC_MB.addCallbackForSortEnd( 'EXC_MB_wysiwyg', function( el ) {
