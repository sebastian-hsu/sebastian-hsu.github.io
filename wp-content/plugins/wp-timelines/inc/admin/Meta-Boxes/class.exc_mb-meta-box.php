...
class EXC_MB_Meta_Box {
	...
	public function init_fields( $post_id = 0 ) {
		...
		foreach ( $this->_meta_box['fields'] as $key => $field ) {
			...
			$class = _exc_mb_field_class_for_type( $field['type'] );
	...
	function enqueue_scripts() {
		...
		wp_enqueue_script( 'exc_mb-scripts', trailingslashit( EXC_MB_URL ) . 'js/exc_mb.js', array( 'jquery' ) );

		wp_localize_script( 'exc_mb-scripts', 'EXC_MBData', array(
			'strings' => array(
				'confirmDeleteField' => __( 'Are you sure you want to delete this field?', 'exc_mb' )
			)
		) );
	...
	function enqueue_styles() {
		...
		$suffix = EXC_MB_DEV ? '' : '.min';

		if ( version_compare( get_bloginfo( 'version' ), '3.8', '>=' ) )
			wp_enqueue_style( 'exc_mb-styles', trailingslashit( EXC_MB_URL ) . "css/dist/exc_mb$suffix.css" );
		else
			wp_enqueue_style( 'exc_mb-styles', trailingslashit( EXC_MB_URL ) . 'css/legacy.css' );
	...
	function add_for_id( $display ) {
		...
		// Don't show EXC_MB if we can't identify ID of a post
	 ...
	 *
	 * @param  EXC_MB_Field[]  $fields
	 */
	static function layout_fields( array $fields ) { ?>

		<div class="exc_mb_metabox">
			...
			foreach ( $fields as $field ) :
				...
				<div class="exc_mb-cell-<?php echo intval( $field->args['cols'] ); ?>">

						<div <?php echo implode( ' ', $attrs ); ?>>
							<?php $field->display(); ?>
						</div>

						<input type="hidden" name="_exc_mb_present_<?php esc_attr_e( $field->id ); ?>" value="1" />
	...
	function strip_repeatable( $values ) {
		...
		foreach ( $values as $key => $value ) {
			...
			if ( false !== strpos( $key, 'exc_mb-group-x' ) || false !==  strpos( $key, 'exc_mb-field-x' ) )
				unset( $values[$key] );
	...
	function save( $post_id = 0 ) {
		...
		foreach ( $this->_meta_box['fields'] as $field ) {
			...
			if ( ! isset( $_POST['_exc_mb_present_' . $field['id'] ] ) )
				continue;
			...
			if ( ! $class = _exc_mb_field_class_for_type( $field['type'] ) ) {
				do_action( 'exc_mb_save_' . $field['type'], $field, $value );
			}
